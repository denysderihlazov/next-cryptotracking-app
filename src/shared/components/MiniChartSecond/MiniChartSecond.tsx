...
import { MiniChartSecondProps } from "./MiniChartSecond.props";
import styles from "./MiniChartSecond.module.css";
...
export const MiniChartSecond = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await fetch(`https://api.coingecko.com/api/v3/coins/tron/market_chart?vs_currency=usd&days=${timeFormat}`)
        ...
        datasets: [
            {
                ...
                borderColor: (context: ScriptableContext<"line">) => {
                    ...
                    gradient.addColorStop(0, "rgba(229,72,158,255)");
                    gradient.addColorStop(1, "rgba(255,135,123,255)");
                    return gradient;
                },
    ...
    return (
        <div
            ...
            {...props}
        >
            <div className={styles.info}><P size="s">TRX <FaExchangeAlt />USDT</P>     <P>{((parseFloat(coinPrice[coinPrice.length -1]) - (parseFloat(coinPrice[0]))) / (parseFloat(coinPrice[0]))*100).toFixed(2)}%</P></div>
